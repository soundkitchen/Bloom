...
BloomApp(.app / AppKit + MetalKit)─ 薄い殻
  ├─ AppDelegate       ウィンドウ構成(通常起動 / 検証モードで分岐)
  ├─ CanvasView        MTKView。入力 → コア API、毎フレーム描画
  ├─ InspectorView     右パネル(ブラシ設定 GUI、レイヤー枠)
  └─ BloomCore(.framework / AppKit 非依存)─ 本体
       ├─ SimulationEngine  滲みシミュレーション + 描画(Metal compute)
...
通常起動は **中央キャンバス + 右インスペクタ + 下ステータスバー**(手動フレームレイアウト、ウィンドウ固定サイズ)。
...
### レイヤー(順序が効くアフィングレーズ合成)

- 各レイヤーは独自の `D`(沈着顔料 float3)+ `visible` + `opacity` を持つ。ウェットシミュレーション(`W`/`P`)は全レイヤー共有
