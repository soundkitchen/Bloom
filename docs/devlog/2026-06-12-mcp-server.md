...
## 追記 3: 21 分スタックの原因究明と送信混線の修正(2026-06-13)

実機テストで「ツールが返ってこないままスタックした」という報告。セッションのトランスクリプトと
Claude Code 側の MCP ログ(`~/Library/Caches/claude-cli-nodejs/.../mcp-logs-bloom/`)を突き合わせて
原因を特定した。決定打はクライアントログのこの 2 行:

```
17:55:07 Ignoring non-JSON line on stdout: JSON Parse error: Expected '}'
17:55:36 Tool 'snapshot' still running (30s elapsed) … (以後 21 分間)
```

**サーバは応答を返していたが、並列ツール呼び出しの 2 応答が混線して壊れた JSON になり、
クライアントが読み捨てていた**。原因は流用していた SDK `StdioTransport.send` の actor 再入:
EAGAIN(送信バッファ満杯)で `await Task.sleep` する間に別ハンドラの send が割り込み、
巨大応答(snapshot は 1 行 ~840KB → EAGAIN が必ず起きる)のバイト列に混ざる。
並列呼び出し + 大きな応答のときだけ起きる競合で、初回の並列呼び出しが無事だったのは運だった。

**修正**:

1. **`MCPSocketTransport` を自作**(皮肉にも当初計画どおり) — send はキューに積むだけにし、
   単一のドレインループだけが fd へ書く。suspension 中の再入はキューが伸びるだけで混線しない
2. **接続を newest-wins に** — 旧仕様「2 本目は即 close」は、死んだセッションのブリッジが
   スロットを握ると新セッションが永遠に繋がらない(これも実害が出た)。新しい接続が来たら
   旧接続を shutdown して新しい方を生かす。takeover は shutdown のみ、close は serve 側に
   一本化(fd 番号の再利用レースを回避)
3. **ブリッジの half-close 対応** — stdin EOF で即死せず、書き込み側だけ shutdown して
   サーバからの送信途中の応答を stdout へ流し切ってから畳む

**検証**: 再現テスト `scripts/mcp-parallel-smoke.py`(snapshot 2 連発を 3 秒読まずに放置 →
全行が JSON として無傷で届くか)を新設し、mcp-smoke に組み込み。修正前ビルドでは
1 回目が混線・2 回目以降は無応答(スロット占有)と**実セッションの事象がそのまま再現**し、
修正後は 3 連続パス(再接続 = newest-wins の検証も兼ねる)。テスト 54 件・スモークもパス。

教訓: **actor は suspension 中に再入する**。「actor だから send は直列」は、途中に await が
挟まる処理では成り立たない。バイト列の原子性が要るところは単一コンシューマのキューにする。
