...
8. [AI と一緒に描く(MCP)](#8-ai-と一緒に描くmcp)
9. [ショートカット & メニュー早見表](#9-ショートカット--メニュー早見表)
10. [困ったとき(トラブルシュート)](#10-困ったときトラブルシュート)
...
| **MP4** | Cmd+Shift+M | 全フレームの H.264 動画。**タイムラインの fps に追従**して書き出します(再生どおりの速さ) |
...
- **🗑 削除**: 現在フレームを削除。
- **fps**: 再生速度(既定 12)。プレビュー再生のテンポを決めます。**MP4 書き出しはこの fps に追従**します(GIF 書き出しは現状 12fps 固定)。
...
Bloom は MCP サーバを内蔵しています。Claude Code などの AI エージェントが、**あなたがいま開いているキャンバスに直接描けます**。線が生え、滲み、乾いていく様子はそのままライブで見えます。

使い方:

1. `make build` でアプリとブリッジをビルドし、`make run` でアプリを起動しておく(ウィンドウは開いたまま、隠さない)
2. このリポジトリで Claude Code を開く。`.mcp.json` のサーバ「bloom」が検出されるので承認する
3. あとは話すだけ。例:「キャンバスの状態を見て、水彩で夕焼けのウォッシュを描いて。乾いたらスナップショットを見せて」

知っておくこと:

- エージェントは描くたびに縮小プレビューを受け取って自己修正します(「いま描いたのを消して描き直して」も undo で通じます)
- エージェントが描いている間、マウスでの描画は一時的に効きません(終われば戻ります)
- エージェントのストロークも 1 本 = 1 回の取り消し単位です。気に入らなければ Cmd+Z で消せます
- **ウィンドウが他のウィンドウに完全に隠れるとシミュレーションが止まり**、滲み・乾燥が進みません。エージェントが「乾かない」と言ってきたらウィンドウを前に出してください
- エージェントを使わないときの動作は何も変わりません。無効にしたい場合は `--no-mcp` を付けて起動します
